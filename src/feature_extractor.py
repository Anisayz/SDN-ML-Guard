...
def flow_to_features(flow: Any) -> dict[str, float]:
    """Convert a completed nfstream NFlow to the 72-feature dict for predict.py."""
    ...
    features["Flow Duration"] = duration_ms * 1000.0   # nfstream=ms, CIC=µs
...
def flow_to_meta(flow: Any) -> dict[str, Any]:
    """Extract flow identity fields for alert.py (not fed to ML model)."""
    return {
